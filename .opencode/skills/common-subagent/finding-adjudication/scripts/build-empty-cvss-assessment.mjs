...
import { cvssAssessmentManifestDigest } from "./cvss-assessment-contract.mjs";
...
async function main() {
  ...
  if (supported.length !== 0) throw new Error("Empty CVSS assessment is allowed only when adjudication has no supported findings");
  const output = {
    schema_version: 1,
  ...
  output.manifest_digest = cvssAssessmentManifestDigest(output);
  const outputPath = resolve(args.output);
  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, `${JSON.stringify(output, null, 2)}\n`, "utf8");
  process.stdout.write(`${JSON.stringify({ output: outputPath, audit_id: output.audit_id, assessments: 0 })}\n`);
}
