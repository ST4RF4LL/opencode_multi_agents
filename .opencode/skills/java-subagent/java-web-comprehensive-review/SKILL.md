...
File and function records must list all `D1` through `D10` in `dimensions_reviewed`. Catalog records use the dimensions declared by that catalog entry and `domain=java`. All records cite concrete evidence and are validated in the trusted structural pass; completed packet claims additionally require an accepted local DONE/GAP handoff.
