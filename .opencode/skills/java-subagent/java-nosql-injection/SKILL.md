...
# Java NoSQL Injection Detection Skill

## Goal

识别攻击者可控输入流向 NoSQL（以 MongoDB 为主）查询构造/执行点，且缺乏有效 API 查询构建器或运算符/特殊字符约束的可达路径。
...
1. **L0 技术栈识别**：识别 MongoDB Java driver / Spring Data MongoDB / 相关 query builder，加载对应 framework model。
2. **L1 候选点定位**：执行 sink locator（grep → Semgrep → Joern），输出 `SinkCandidate`，不是漏洞。
3. **L2 数据流分析**：从 sink 反向扩展调用链；执行 source-to-sink 可达分析。
4. **L3 防护分析**：识别 Filters/Bson API、运算符拒绝、类型约束、JSON 解析边界；按 sanitizer 分级评估。
...
1. **rg**：技术栈与危险 API 关键词
2. **Semgrep**：局部危险模式（Document.parse、字符串拼接查询、$where、JSON 运算符注入）
3. **Joern**：跨方法调用链与数据流
4. **CodeQL**：可构建项目的补充分析
5. **LLM**：业务语义、运算符注入、JSON 结构控制、防护有效性研判
...
- 未发现相关 NoSQL sink
- sink 查询完全由常量/安全 API（Filters.eq 等）构造且用户输入仅作绑定值
- 所有可达路径均经有效 query builder 或完整运算符/特殊字符拒绝
- 无法建立外部输入到危险 NoSQL 查询构造的可信路径
...
- Audit dimension: **D1** / weakness `nosql-injection`.
- Progressive-load sibling assets in this skill directory (`models/`, `rules/`, `analysis/`, `cases/`, `validation/`, `evidence/`).
- Published Joern rules live under `.opencode/shared/security-audit/joern-rules/java/` with ids `java-nosql-injection-*`.
- Reusable cases live under `.opencode/shared/security-audit/vulnerability-cases/java/java-nosql-injection-case-*`.
- False-positive patterns live under `.opencode/shared/security-audit/false-positive-cases/java/java-nosql-injection-fp.md`.
