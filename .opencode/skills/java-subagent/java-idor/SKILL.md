...
识别攻击者可控的对象标识（path/query/body 中的 id、uuid、orderId、userId、tenantId 等）流向资源加载/更新/删除操作，且缺乏有效对象级授权（ownership / tenant / role 校验）的可达路径。以 create/add 时建立的 owner、user、tenant、role、project、workspace 或 permission scope 为基线，逐项审查同类资源的 read/list/query/update/delete/export/execute/cancel/batch。对于语义上依赖父资源 A 的子资源 B，必须同时证明调用者获准操作 A，以及 B 与该 A 的归属关系被服务端绑定。认证存在 ≠ 对象级授权存在；嵌套路由存在 ≠ 父子授权绑定存在。
...
4. **L3 防护分析**：先从 create/add、entity mapping、repository schema 归纳资源归属边界，再建立 `resource × operation × required scope × actual control` 矩阵；识别 ownership 比较、tenant 过滤、`@PreAuthorize`、SecurityExpression、查询级 `and userId = ?`；对 A → B 审查 `authorize(A)`、`B belongsTo A` 与 CRUD 操作是否在同一服务端信任链内绑定；批量操作逐项检查；按 sanitizer 分级评估。
...
2. **Semgrep/OpenGrep**：通过受控 `.opencode/scripts/semgrep-scan.mjs` CLI 执行本地兼容规则，定位路径 id 直查、删除无 owner、client tenantId
3. **Joern**：跨方法调用链与数据流
...
- create/add 将 `userId`、owner、tenant 或权限域写入资源，是后续操作需要对象级约束的强语义证据；客户端提交的归属字段不是可信授权上下文
- 查询/列表只有在明确的共享或公开策略下才能 Rejected。若确为只读且无写/删能力，可结合数据敏感性降低影响评级，但不能因此忽略跨用户或跨租户读取
- 无完整证据时输出 **Candidate**，不得直接报告 Finding
- 每个 Finding 必须回答：create/add 建立了什么归属边界、object id 从哪来、访问了什么资源、中间经过什么、该具体操作为何缺少等价对象级授权、攻击者可读写谁的对象、如何证明可达
