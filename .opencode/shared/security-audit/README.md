...
3. The orchestrator validates catalog v2, freezes a sparse Focus-Area-bound Coverage Plan, and initializes the service-owned append-only Coverage Ledger. Source, platform, and AI auditors run Focus Area `coverage` sessions under each Tri-Lens strategy, recording compact server-derived required-source-set receipts and decisions through `coverage_ledger`; full source lists stay server-side and appear only in small targeted diagnostic pages. They also run checklist-light `blind` and history/case-seeded `seeded-variant` discovery where required.
