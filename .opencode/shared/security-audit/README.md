...
新任务的 AI 深度审计以 Recon 的 scope.ai_routing 为准：全仓有据筛查，相关组件/依赖/未知项/负面样本进入深审，排除项不是 AI PASS。真实性复核共享不可变事实包，保留独立结论与 FULL 升级条件。
