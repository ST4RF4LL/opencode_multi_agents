// L2: source-to-sink dataflow for path traversal candidates.
...
val annotatedSources = cpg.parameter
  .where(_.annotation.name("RequestParam|PathVariable|RequestHeader|RequestBody|RequestPart"))
...
val sinks = cpg.call.methodFullName(
  ...
  ".*Files\\.delete.*|" +
  ".*Files\\.copy.*|" +
  ".*FileUtils\\.(readFileTo|write|copyFile|openInputStream|openOutputStream).*"
).argument
...
flows.zipWithIndex.foreach { case (flow, idx) =>
  println(s"dataflow_id=PATH-FLOW-$idx")
