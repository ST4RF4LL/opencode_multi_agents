...
async function writeSemanticInputs(directory) {
  ...
  await writeFile(threatPath, `${JSON.stringify({
    ...
    security_invariants: [{
      ...
      statement: "A tenant may read only records owned by that tenant.",
      asset_ids: ["ASSET-001"],
      threat_ids: ["T-001"],
      enforcement_points: ["tenant ownership check"],
      evidence: [{ fixture: "ownership-policy" }],
      provenance_tags: ["code-verified"],
    }],
    assumptions: [{
      assumption_id: "ASM-001",
      statement: "The gateway preserves the authenticated tenant identity.",
      category: "deployment",
      status: "VERIFIED",
      affects_threat_ids: ["T-001"],
      evidence: [{ owner_answer: "yes" }],
      provenance_tags: ["owner-asserted"],
    }],
    attacker_stories: [{
      story_id: "STORY-001",
      actor_id: "ACTOR-001",
      ...
      preconditions: ["The caller has a valid tenant account."],
      steps: ["Call the tenant API with another tenant's record ID.", "Reach the record lookup without an ownership decision."],
      outcome: "The caller reads another tenant's sensitive record.",
      evidence: [{ fixture: "tenant-flow" }],
      provenance_tags: ["code-verified"],
    }],
    out_of_scope_stories: [{
      story_id: "OOS-001",
      scenario: "Compromise of the managed gateway control plane.",
      reason: "The gateway implementation is not present in the source scope.",
      reconsider_when: ["Gateway source or deployment evidence is added to scope."],
      evidence: [{ fixture: "scope-boundary" }],
      provenance_tags: ["deployment-unknown"],
    }],
    severity_calibration: {
      model: "contextual-four-level-v1",
      context_notes: ["Final supported findings receive separately derived CVSS 3.1 scores."],
      evidence: [{ fixture: "severity-policy" }],
      levels: [
        { severity: "CRITICAL", criteria: ["Cross-tenant compromise at broad scale or administrative control loss."], examples: [{ scenario: "A tenant can enumerate all tenant records.", rationale: "Broad cross-tenant confidentiality impact.", threat_ids: ["T-001"] }], not_applicable_reason: null },
        { severity: "HIGH", criteria: ["Repeatable cross-tenant access to sensitive records."], examples: [{ scenario: "A tenant reads a selected other-tenant record.", rationale: "Direct sensitive-data boundary failure.", threat_ids: ["T-001"] }], not_applicable_reason: null },
        { severity: "MEDIUM", criteria: ["Impact is constrained to nonsensitive metadata or strong preconditions."], examples: [{ scenario: "Only record existence metadata is disclosed.", rationale: "Limited confidentiality impact.", threat_ids: ["T-001"] }], not_applicable_reason: null },
        { severity: "LOW", criteria: ["No sensitive record is exposed and exploitation requires exceptional conditions."], examples: [{ scenario: "A rejected identifier produces a distinguishable error only.", rationale: "Minimal observable effect.", threat_ids: ["T-001"] }], not_applicable_reason: null },
      ],
    },
    deprioritized: [],
