...
测试环境是否可用于本次验证由用户自行判断并显式授权，贯穿式流程不根据公网、内网或本机地址分类拒绝目标。支持 IP、域名和 IPv6，只校验 HTTP(S) 地址格式、身份材料及授权目标范围；不会为了判断网络类别预先连接目标。

匿名环境可直接填写 `URL: http://test.example:8080` 或 `地址：192.0.2.10:8080`。省略协议时默认 HTTP，HTTPS 须显式填写。说明中有多个链接时，以 `URL:`、`地址：` 等字段指定主地址；没有主地址字段时，同一 origin 的多个链接可正常解析，不同 origin 则需要明确指定。需要登录时必须选择对应身份模式，分行填写 `测试账号:` 和 `测试密码:`。多身份或持久化测试使用 JSON，例如：
...
勾选“创建并清理测试记录”时，测试数据范围和清理说明也是必要信息。工作包不能自行扩大这份数据范围。默认仅授权主地址的 origin；跨主机、协议或端口的登录、API、子资源须在 JSON `origins` 中明确列出完整 origin（例如 `["http://test.example:8080", "https://login.test.example"]`，须包含主地址）。未授权的跳转和子资源仍被代理拒绝。不同测试身份使用独立 Chrome DevTools MCP 浏览器，禁止接管用户已有 Chrome。

地址缺失、格式无效、主地址不明确、origins 无效和缺少账号分别展示具体原因，不回显原始敏感上下文。已创建任务的授权和证据不会被修改；更新后需新建任务应用新规则。旧版 quick 和人工补充验证继续按各自历史契约处理。
