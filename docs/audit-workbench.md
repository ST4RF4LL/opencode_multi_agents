...
动态验证分成两层。创建任务时未启用“测试环境信息”，审计主链不会启动浏览器，所有初步支持项直接进入本地三方静态复核；启用后，主链获准执行一次、共享环境准备最多 240 秒、每个疑似漏洞报告最多 180 秒的 loopback 快速动态确认，未确认项仍进入三方。完整动态验证不属于普通审计的自动阶段：操作员还要启用动态 Runner，并在“完整动态验证”页面逐次明确请求、确认授权的 localhost 环境；账号及登录/清理步骤按目标实际需要选填。完整结果是 sidecar，不自动改写 routing、finding、攻击链或终稿。
...
审计任务页提供“运行中 / 已完成 / 全部”三个分类，默认进入“运行中”。准备中、恢复中、运行中、正在暂停和正在取消属于运行过程，不分页；其余状态均可在“已完成”中检索，包括排队中、已暂停、中断、取消、失败和历史制品。“已完成”与“全部”默认每页 20 条，可切换 30、50、100 条。搜索按任务名称、ID、仓库名称和提交在服务端筛选，然后分页；越界页自动回到最后一页。

`GET /api/v1/audits` 接受 `tab=running|completed|all`、`page`、`page_size` 和 `q`，返回当前页轻量摘要及 `count/page/page_size/total_pages`；未指定分类时默认查询全部的第一页。前端进入任务页、切换分类、搜索或翻页时才请求列表，只保留当前页；切换请求会取消旧请求，详情点击后通过单任务接口加载。前端使用 `GET /api/v1/workspace?audits=compact`，只携带最近五条任务及 SSE 订阅需要的一条活动任务摘要，避免工作区快照再次传输全部任务详情。其他工作区资源和服务端制品聚合仍沿用现有机制。

前端会连接活动审计的 SSE 流并节流刷新统一快照，任务页可见时同步刷新当前页和已选详情；断线时仍可通过手工刷新从持久制品恢复。

列表与自动刷新使用 `live=1` 展示读取：服务端只保留一份工作区快照，3 秒后由下一次读取触发单次后台重建，已有页面无需等待扫描结束；任务状态、操作版本号与队列状态合并 Runner 当前值。制品数量、阶段校验与发现统计可能滞后一个重建周期，首次无缓存仍需完成扫描。项目就绪检查在展示读取中复用最多 10 秒，实际创建任务时仍重新检查。写请求前后使快照失效，失效前开始的扫描不能回填缓存；未带 `live=1` 的接口及删除、报告读取等路径仍等待新鲜校验。

前端同一查询的后台刷新保留已有列表及详情，查询条件变化才清空旧列表。工作区、任务列表和详情并行请求，不再串行触发两轮扫描。首屏各资源独立到达后渲染；运行环境探测及动态验证记录只在对应页面加载。本改动减少重复扫描与刷新空白，不承诺首次扫描的固定延迟。
...
快速动态的预算、共享会话和失败处理详见[共享环境说明](quick-dynamic-shared-environment.md)。
