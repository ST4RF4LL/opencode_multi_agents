...
在“设置”的“使用模型”中可为 Web 任务选择 `默认` 或已配置的 `provider/model`。清单从工作台的 `.opencode/opencode.json`、兼容路径 `~/.config/opencode.json` 及 OpenCode 标准全局配置 `~/.config/opencode/opencode.json` 读取；选择具体模型后，工作台会把它固化到新建审计，并以 `opencode run --model provider/model` 启动，因此后续切换交互式 OpenCode 的默认模型不会影响该审计或其断点恢复。
